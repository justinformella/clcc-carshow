...
export async function POST(request: NextRequest) {
  try {
    const { registration_id, type, model, quality, background } = await request.json();
    ...
    let t0 = Date.now();
    const rawBuffer = await generateImage(prompt, aspectRatio, model || undefined, quality || "medium", background || "auto");
