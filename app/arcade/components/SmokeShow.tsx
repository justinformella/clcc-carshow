...
const IVY_HALL_LOGO = "https://vuwiucgxxaoygsyxqodk.supabase.co/storage/v1/object/public/pixel-art/8bit/sponsor-9c0e0de3-ab93-4298-a81e-e123360ee911.png";
...
export default function SmokeShow({ playerCar, onBack }: SmokeShowProps) {
  ...
  const driverPhaseRef = useRef<"hidden" | "exitCar" | "walkRight" | "inside" | "walkLeft" | "enterCar">("hidden");
  ...
  const drawDriver = useCallback((ctx: CanvasRenderingContext2D, x: number, groundY: number, facingLeft: boolean, frame: number) => {
    ...
    if (frame % 2 === 0) {
      ctx.fillStyle = "#333";
      ctx.fillRect(facingLeft ? x - 3 : x + 1, y + 16, 3, 3);
    } else {
      ctx.fillStyle = "#333";
      ctx.fillRect(facingLeft ? x + 1 : x - 3, y + 16, 3, 3);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (carImg?.complete && carImg.naturalWidth > 0) {
        ...
        if (phase === "arriving") {
          ...
          if (carXRef.current < targetX) {
            ...
            if (carXRef.current >= targetX) {
              ...
              driverXRef.current = carXRef.current + carW * 0.55;
        ...
        if (phase === "visiting") {
          ...
          if (driverPhaseRef.current === "exitCar") {
            // Brief pause then start walking
            if (visitTimerRef.current > 20) {
              driverPhaseRef.current = "walkRight";
              driverFrameRef.current = 0;
            }
          } else if (driverPhaseRef.current === "walkRight") {
            driverXRef.current += 2;
            driverFrameRef.current++;
            if (driverXRef.current >= doorX) {
              driverPhaseRef.current = "inside";
              visitTimerRef.current = 0;
            }
          } else if (driverPhaseRef.current === "inside") {
            if (visitTimerRef.current > 120) { // ~2 seconds
              driverPhaseRef.current = "walkLeft";
              driverXRef.current = doorX;
              driverFrameRef.current = 0;
            }
          } else if (driverPhaseRef.current === "walkLeft") {
            driverXRef.current -= 2;
            driverFrameRef.current++;
            const carDoorX = carXRef.current + carW * 0.55;
        ...
        if (phase === "burnout") {
          // RPM decay
          rpmRef.current = Math.max(800, rpmRef.current - 50);
          ...
          const spawnRate = Math.floor(rpmRef.current / 1000);
          const tireX = playerCar.flipped ? carX + carW - 10 : carX + 10;
          const tireY = groundY - 5;
          for (let i = 0; i < spawnRate; i++) {
            const hot = rpmRef.current > redline * 0.8 && Math.random() < 0.2;
            particlesRef.current.push({
              x: tireX + (Math.random() - 0.5) * 10,
              y: tireY + (Math.random() - 0.5) * 5,
              vx: (playerCar.flipped ? 1 : -1) * (1 + Math.random() * 2.5),
        ...
        if (phase === "launching") {
          ...
          // Trailing smoke
          const tireX = playerCar.flipped ? carX + carW : carX;
          const tireY = groundY - 5;
          for (let i = 0; i < 4; i++) {
            particlesRef.current.push({
              x: tireX, y: tireY,
        ...
        // Draw driver
        if (driverPhaseRef.current === "walkRight" || driverPhaseRef.current === "exitCar") {
          drawDriver(ctx, driverXRef.current, groundY, false, Math.floor(driverFrameRef.current / 8));
        } else if (driverPhaseRef.current === "walkLeft" || driverPhaseRef.current === "enterCar") {
          drawDriver(ctx, driverXRef.current, groundY, true, Math.floor(driverFrameRef.current / 8));
        }
      }
      ...
      if (phaseRef.current === "burnout") {
        const gaugeH = 8, gaugeY = H - gaugeH - 8, gaugeX = 12, gaugeW = W - 24;
    ...
    <div style={pageStyle}>
      <div style={{ textAlign: "center", marginBottom: "0.25rem" }}>
        <a href="https://www.ivyhallil.com" target="_blank" rel="noopener noreferrer" style={{ textDecoration: "none", display: "inline-block" }}>
          <img src={IVY_HALL_LOGO} alt="Ivy Hall" style={{ maxWidth: "120px", height: "auto", imageRendering: "pixelated", display: "block", margin: "0 auto 0.3rem" }} />
