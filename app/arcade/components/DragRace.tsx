...
import {
  ...
  calibratePlayer, calibrateOpponent,
} from "@/lib/race-physics";
import { RaceCar, C, FONT, pageStyle, goldBtnStyle, pixelBtnStyle, kbdStyle } from "@/lib/race-types";
...
// ─── Component ───
type DragPhase = "matchup" | "countdown" | "racing" | "finished";
  ...
  const startRace = useCallback(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (c <= 0) {
        ...
        // If player jumped, add 0.5s penalty
        const jumpPenalty = jumpedRef.current ? 500 : 0;
        startRef.current = performance.now() - jumpPenalty;
        ...
        const animate = (now: number) => {
          ...
          if (accel && !pFinish) {
            ...
            const rpmPct = (pRpm - 800) / (pRedline - 800);
          ...
          // Opponent AI
          if (!oFinish) {
            const oppElapsed = elapsed - oppRT;
      ...
      <div style={pageStyle}>
        ...
        <div style={{ maxWidth: "800px", margin: "0 auto" }}>
          ...
          <div style={{ textAlign: "center" }}>
            ...
            <br />
            <button onClick={onBack} style={{ background: "none", border: "none", color: C.midGray, fontFamily: FONT, fontSize: "1rem", cursor: "pointer", textDecoration: "underline" }}>
              BACK TO GARAGE
            </button>
          </div>
